...
func (r *Runner) Run(ctx context.Context, configs []*grpcv1.LoadTest, suiteReporter *TestSuiteReporter, concurrencyLevel int, outputDir, logURLPrefix string, done chan<- *TestSuiteReporter) {
	var count, n int
	...
	for _, config := range configs {
		...
		go r.runTest(ctx, config, reporter, outputDir, logURLPrefix, testDone)
	}
	for n > 0 {
		reporter := <-testDone
		reporter.SetEndTime(time.Now())
...
func (r *Runner) runTest(ctx context.Context, config *grpcv1.LoadTest, reporter *TestCaseReporter, outputDir, logURLPrefix string, done chan<- *TestCaseReporter) {
	var s, status string
	var retries uint
